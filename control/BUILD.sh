...
cat $pert/combo/$model/predictions${train}.${test}/* | cut -f-2 | awk 'BEGIN {FS = "_"} {print $2 "\t" $0}' | sort -k1,1 | join -t$'\t' - <(cat $pert/combo/$model/predictions${train}.${test}/* | cut -d_ -f2 | sort | uniq -c | awk '{print $2 "\t" 1 / $1}' | sort -u | sort -k1,1) | sed 's/_/\t/g' | awk 'BEGIN {FS = "\t"} {print $2 "_" $3 "_" $4 "\t" $7 "\t" $8}' | sort -k1,1 | join -t$'\t' <(cat $pert/$testname/labeled_pairs.txt | cut -f1 | awk 'BEGIN {FS = "_"} {print $1 "_" $2 "_" $3 "\t" $4}' | sort -k1,1) - | sed 's/POSITIVE/1/g' | sed 's/NEGATIVE/0/g' | awk 'BEGIN {FS = "\t"} {if($3 == 1){$3 = 1} else{$3 = 0} print $2 "\t" $3 "\t" $4}' | awk 'BEGIN {FS = "\t"; tp = 0; fp = 0; fn = 0} {if($1 == 1 && $2 == 1){tp += $3} else if($1 == 1){fn += $3} else if($2 == 1){fp += $3}} END {print tp "\t" fp "\t" fn}'; done | paste -sd$'\t' | awk '{print "'$modelname'+'$comboname'\t" $0}'; done
