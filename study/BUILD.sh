...
(cat ../corpora/CREEDS/single_gene_fixed.txt | awk 'NR > 1' | cut -f6,11 | sed 's/ /#/g' | sort -t$'\t' -k2,2 | join -t$'\t' -1 2 -2 1 - <(cat ../corpora/CREEDS/fixed_kokd_terms.txt | sed 's/ /#/g' | sort -u) | cut -f2 | sort -u | awk '{print $1 "\t1"}'; (cat ../corpora/CREEDS/single_gene_fixed.txt | awk 'NR > 1' | cut -f6; cat ../corpora/CREEDS/single_drug_fixed.txt | awk 'NR > 1' | cut -f7) | sort -u | comm -23 - <(cat ../corpora/CREEDS/single_gene_fixed.txt | awk 'NR > 1' | cut -f6,11 | sed 's/ /#/g' | sort -t$'\t' -k2,2 | join -t$'\t' -1 2 -2 1 - <(cat ../corpora/CREEDS/fixed_kokd_terms.txt | sed 's/ /#/g' | sort -u) | cut -f2 | sort -u) | awk '{print $1 "\t0"}') | sort -u > gene/CREEDS/labels.txt
