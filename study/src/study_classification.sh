...
python3 $srcdir/chew.py <(cat $labelfile | sort -t$'\t' -k1,1 | join -t$'\t' - <(cat $studyinfofile | sed 's/$SEMICOLON$/;/g' | grep -v 'Refer to individual Series. This SuperSeries is composed of the SubSeries listed below.' | sort -t$'\t' -k1,1) | awk 'BEGIN {FS = "\t"} {print $1 "_" $2 "\t" $3}') $model | awk 'BEGIN {FS = "\t"} {gsub("_", "\t", $1); print $1 "\t" $2 "\t" $3}' | awk 'BEGIN {FS = "\t"} {print $1 "_" $3 "\t" $2 "\t" $4}' | sed 's/"//g' | sed 's/\\//g' | sort -u > $OUT_DIR/study_info_chewed.txt
...
for split in $(ls $splitdir | grep -vw $trial); do cat $splitdir/$split; done | sort -u | join -t$'\t' - <(cat $OUT_DIR/study_info_chewed.txt | awk 'BEGIN {FS = "_"} {print $1 "\t" $0}' | sort -t$'\t' -k1,1) | cut -f3- | awk 'BEGIN {FS = "\t"} {print "{\"text\":\"" $2 "\",\"label\":" $1 "}"}' | sort -u | shuf --random-source=<(openssl enc -aes-256-ctr -pass pass:"2025" -nosalt </dev/zero 2>/dev/null) > $OUT_DIR/training_datasets/$trial.json
done

mkdir $OUT_DIR/testing_datasets
for split in $(ls $splitdir); do
cat $OUT_DIR/study_info_chewed.txt | awk 'BEGIN {FS = "_"} {print $1 "\t" $0}' | sort -t$'\t' -k1,1 | join -t$'\t' - <(cat $splitdir/$split | sort -u) | cut -f2- | awk 'BEGIN {FS = "\t"} {print $1 "_" $2 "\t" $3}' > $OUT_DIR/testing_datasets/$split.txt
done
